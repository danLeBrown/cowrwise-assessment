...
async def get_users(db: Session = Depends(get_admin_db), redis_client: RedisService = Depends(get_redis)):
...
# Fetch / List user's borrowed books.
@app.get("/users/{user_id}/borrowed-books", response_model=UserBorrowedBooksSchema)
async def get_users(user_id: str, db: Session = Depends(get_admin_db), redis_client: RedisService = Depends(get_redis)):
    user_service = UserService(redis_client, UserRepo(db))
    return user_service.find_with_borrowed_books(user_id)

# Add new books to the catalogue
@app.post("/books", response_model=BookSchema)
async def create_book(book: CreateBookSchema, db: Session = Depends(get_admin_db), redis_client: RedisService = Depends(get_redis)):
...
async def get_books(db: Session = Depends(get_admin_db), redis_client: RedisService = Depends(get_redis)):
    book_service = BookService(redis_client,book_repo=BookRepo(db), borrowed_book_repo=BorrowedBookRepo(db))
...
async def get_borrowed_books(db: Session = Depends(get_admin_db), redis_client: RedisService = Depends(get_redis)):
    book_service = BookService(redis_client, book_repo=BookRepo(db), borrowed_book_repo=BorrowedBookRepo(db))
    return book_service.find_borrowed_books()

# Remove a book from the catalogue.
@app.put("/books", response_model=UpdateSchema)
async def update_book(request: UpdateBookSchema, db: Session = Depends(get_admin_db), redis_client: RedisService = Depends(get_redis)):
    book_service = BookService(redis_client, book_repo=BookRepo(db), borrowed_book_repo=BorrowedBookRepo(db))
